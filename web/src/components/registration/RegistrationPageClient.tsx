...
import { LocaleProvider, useDisplayCopy, useRegistrationCopy } from "@/components/locale/LocaleProvider";
...
function RegistrationVerifiedShell({
  account,
  locale,
  onLocaleChange,
}: {
...
export function RegistrationPageClient({
  ...
  const gateCopy = REGISTER_GATE_COPY[locale];
        ...
        <main className={`${appContentClass} min-w-0 px-3 py-8 sm:px-4 sm:py-16`}>
          <div className="mx-auto w-full max-w-2xl rounded-2xl border border-teal-100 bg-white p-5 shadow-sm dark:border-teal-900/50 dark:bg-zinc-900 sm:p-8">
            <h1 className="text-xl font-bold text-teal-950 dark:text-teal-100 sm:text-2xl">
              {gateCopy.languageStep.titleBilingual}
            </h1>
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">{gateCopy.languageStep.subtitle}</p>
            <div className="mt-6">
  ...
  return (
    <LocaleProvider locale={locale}>
      <RegistrationVerifiedShell account={account} locale={locale} onLocaleChange={handleLocaleChange} />
    </LocaleProvider>
  );
}
