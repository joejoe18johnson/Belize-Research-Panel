    ...
    <div>
      ...
      <div className="mt-1.5 flex gap-2">
        <input
          ...
          readOnly
          value={value}
          className="min-w-0 flex-1 rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-800"
          onFocus={(event) => event.target.select()}
        />
        <button
          type="button"
          onClick={copy}
          className="shrink-0 rounded-xl border border-teal-200 bg-teal-50 px-4 py-2.5 text-sm font-semibold text-teal-800 hover:bg-teal-100"
        >
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
...
export function CampaignLaunchLinks({
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 sm:p-6">
      <div>
        <h2 className="text-lg font-semibold text-teal-950">{formatHeadingCase("Survey links")}</h2>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          type="button"
          onClick={copyAll}
          className="inline-flex min-h-10 items-center rounded-xl border border-zinc-200 bg-white px-4 text-sm font-semibold text-zinc-700 hover:bg-zinc-50"
        >
          {copiedAll ? "All links copied" : "Copy all links"}
        </button>
      </div>

      <div className="max-h-[28rem] space-y-3 overflow-y-auto rounded-xl border border-zinc-200 bg-white p-3 sm:p-4">
        {surveyLinks.map((row) => (
          <div key={row.panelistEmail} className="rounded-xl border border-zinc-100 bg-zinc-50/80 p-4">
            <p className="text-sm font-semibold text-zinc-900">{row.panelistName}</p>
