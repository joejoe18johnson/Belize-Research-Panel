...
function SectionBlock({ section }: { section: AdminModuleContent["sections"][number] }) {
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
      <h2 className="text-base font-semibold text-teal-950">{formatHeadingCase(section.title)}</h2>
      {section.body ? (
        <p className="mt-3 text-sm leading-relaxed text-zinc-700">{section.body}</p>
      ) : null}
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="border-l-4 border-teal-600 pl-4">
        <h1 className="text-2xl font-bold text-teal-950 sm:text-3xl">{module.label}</h1>
          ...
          <ul className="mt-4 space-y-3">
            {content.liveInPortal.map((item) => (
              <li key={item.label} className="rounded-xl border border-teal-100 bg-white px-4 py-3 text-sm">
                ...
                <p className="mt-1 leading-relaxed text-zinc-600">{item.detail}</p>
              </li>
            ))}
          </ul>
        </section>
      ...
      {content.dataSources?.length ? (
        <section className="rounded-2xl border border-zinc-200 bg-zinc-50/80 p-5 sm:p-6">
          <h2 className="text-base font-semibold text-teal-950">{formatHeadingCase("Data sources")}</h2>
      ...
      {content.plannedNext?.length ? (
        <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-base font-semibold text-teal-950">{formatHeadingCase("Planned next")}</h2>
          <ul className="mt-3 list-disc space-y-1.5 pl-5 text-sm leading-relaxed text-zinc-700">
