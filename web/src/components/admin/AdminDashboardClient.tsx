...
export function AdminDashboardClient({
  ...
  const flagRecord = async (email: string) => {
    ...
    try {
      ...
      setRowActionMessage(`Flagged ${panelistDisplayLabel(rows.find((row) => row.email === email) ?? { email } as PanelistRow)} as Possible Duplicate.`);
      router.refresh();
    } catch {
  ...
  const deleteRecord = async (email: string) => {
    ...
    if (!window.confirm(`Delete panelist record for ${label}? This removes the register entry and related survey data.`)) {
      return;
    }
    ...
    <div className="mx-auto max-w-[1400px] space-y-8">
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        {duplicateActionMessage ? (
          <p className="mt-3 text-sm text-teal-800">{duplicateActionMessage}</p>
        ) : null}
        {duplicateRows.length > 0 ? (
          <>
            <p className="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
              {duplicateRows.length} records need duplicate review. Use row actions to edit, flag, or delete.
            </p>
            <div className="mt-4 overflow-x-auto">
              <DataTable rows={duplicateRows} columns={DUPLICATE_REVIEW_COLUMNS} highlightDuplicates actions={rowActions} />
            </div>
          </>
        ) : (
      ...
      <section className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Edit, flag, or delete from the actions column on each row.</p>
          </div>
          <button
            type="button"
            onClick={exportCsv}
        ...
        {rowActionMessage ? <p className="mt-3 text-sm text-teal-800">{rowActionMessage}</p> : null}
        <div className="mt-4 overflow-x-auto">
          <DataTable rows={filteredRows} columns={TABLE_COLUMNS} highlightDuplicates actions={rowActions} />
        </div>
      </section>

      {editingEmail && editState ? (
        <PanelistEditModal
          label={panelistDisplayLabel(rows.find((row) => row.email === editingEmail) ?? { email: editingEmail } as PanelistRow)}
          editState={editState}
...
function PanelistEditModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-zinc-900/50 p-4 sm:items-center" role="dialog" aria-modal="true">
        ...
        <div className="space-y-6 p-5">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <FieldInput
              ...
              onChange={(value) => onChange({ ...editState, email: value })}
            />
            <FieldInput
...
function RowActionButtons({
  ...
  return (
    <div className="flex items-center gap-0.5">
      ...
      <IconButton
        label="Delete record"
        onClick={() => actions.onDelete(email)}
        disabled={busy}
        tone="red"
      >
        <TrashIcon />
  ...
  return (
    <table className="min-w-full text-left text-xs sm:text-sm">
      ...
      <tbody>
        {rows.map((row, index) => {
          const flagged =
            highlightDuplicates && "duplicate_name_dob_flag" in row && row.duplicate_name_dob_flag;
