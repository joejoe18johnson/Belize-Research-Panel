    ...
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        <div className="flex flex-col lg:flex-row">
          <aside className="border-b border-zinc-100 bg-zinc-50/80 lg:w-64 lg:shrink-0 lg:border-b-0 lg:border-r">
            <div className="border-b border-zinc-100 px-4 py-3 lg:hidden">
              <label className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Topic</label>
              <select
                value={activeTopicId}
                onChange={(event) => setActiveTopicId(event.target.value as SurveyTemplateTopicId)}
                className="mt-1.5 w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm"
              >
                ...
                return (
                  <button
                    ...
                    className={`mb-0.5 w-full rounded-lg px-3 py-2.5 text-left text-sm transition ${
                      active
                        ? "bg-white font-semibold text-teal-900 shadow-sm ring-1 ring-zinc-200"
          ...
          <div className="min-w-0 flex-1 p-4 sm:p-6">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex min-h-9 items-center rounded-lg border border-zinc-200 bg-white px-4 text-sm font-semibold text-zinc-700 hover:bg-zinc-50"
              >
                Start from scratch
              </button>
            </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {templates.map((template) => (
                  <button
                    ...
                    className="group rounded-xl border border-zinc-200 bg-zinc-50/60 p-4 text-left transition hover:border-teal-300 hover:bg-teal-50/40 hover:shadow-sm"
                  >
                    <p className="text-sm font-semibold text-teal-800 group-hover:text-teal-900">{template.title}</p>
                    <p className="mt-2 text-xs text-zinc-500">
                      {template.questions.length} {template.questions.length === 1 ? "question" : "questions"}
                    </p>
                    <p className="mt-2 line-clamp-2 text-xs leading-relaxed text-zinc-600">{template.description}</p>
